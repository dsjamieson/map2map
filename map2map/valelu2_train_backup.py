...
def gpu_worker(local_rank, node, args):
    ...
    model = model(args.style_size, sum(args.in_chan), sum(args.out_chan), **args.misc_kwargs)
    model.to(device)
...
def train(epoch, loader, model, criterion,
    ...
    for i, data in enumerate(loader) :
        ...
        #input = input.cpu().requires_grad_()
        #dis_out = dis_out.cpu().requires_grad_()
        #dis_tgt = dis_tgt.cpu().requires_grad_()
        #$vel_out = vel_out.cpu().requires_grad_()
        #vel_tgt = vel_tgt.cpu().requires_grad_()
        ...
        den_loss = den_loss.to(device, non_blocking=True).requires_grad_()
        dis_loss = dis_loss.to(device, non_blocking=True).requires_grad_()
        mom_loss = mom_loss.to(device, non_blocking=True).requires_grad_()
        vel_loss = vel_loss.to(device, non_blocking=True).requires_grad_()
        mom_loss_2 = mom_loss_2.to(device, non_blocking=True).requires_grad_()

        loss = den_loss * mom_loss * mom_loss_2 * (dis_loss * vel_loss) ** 3
        ...
        optimizer.zero_grad()
        opt_loss.backward()  # NOTE actual loss is log(loss) / (1 + z)
